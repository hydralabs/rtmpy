...
class Decoder(ChannelDemuxer):
    """
    Dispatches decoded RTMP messages to a C{delegate}.

    At this layer, a message is a datatype, a timestamp and a blob of data. It
    is up to the delegate to decide how to handle the decoding of the data.

    @ivar message_delegate: Receives dispatched messages generated by the decoder.
    @ivar stream_factory: Builds stream listener objects.
    """

    def __init__(self, stream_factory, delegate, stream=None):
    ...
    def next(self):
        """
        Iterates over the RTMP stream and dispatches messages to the C{delegate}
        ...
        self.message_delegate.dispatchMessage(stream,
            meta.datatype, stream.timestamp, data)
